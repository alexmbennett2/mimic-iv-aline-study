...
(
  ...
  ( -- begin query that extracts the data
    select co.icustay_id, charttime
        ...
        , case
          when itemid = 52028 then 'SPECIMEN'
...
(
  ...
  (
    case
      when max(oe.charttime) < min(co.intime) and max(oe.charttime) <= min(oe.charttime) then 1
    else
      -- a little bit of Laplacian smoothing never hurt anyone
      DATETIME_DIFF(max(oe.charttime), coalesce(min(oe.charttime),min(co.intime)), HOUR) + 1
    end
  )*24.0 as UrineOutput
...
(
  ...
  , max(case when l.itemid in (223900) then l.valuenum else null end) as GCSVerbal
  , max(case when l.itemid in (220739) then l.valuenum else null end) as GCSEyes
...
(
  ...
  , min(case
      ...
      when b.EndoTrachFlag = 1
        then 15
      when b.GCSVerbal is null and b2.EndoTrachFlag = 1
        then 15
      -- if previously they were intub, but they aren't now, do not use previous GCS values
      when b2.EndoTrachFlag = 1
        then
